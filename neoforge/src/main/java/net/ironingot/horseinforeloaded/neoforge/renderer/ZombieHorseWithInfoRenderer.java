...
public class ZombieHorseWithInfoRenderer extends UndeadHorseRenderer {
    ...
    public void onPostRenderInfo(RenderLivingEvent.Post<ZombieHorse, EquineRenderState, AbstractEquineModel<EquineRenderState>> event) {
        if (!HorseInfoMod.isActive()) {
            return;
        }
   
        if (!(event.getRenderState() instanceof ZombieHorseWithInfoRenderState)) {
            return;
        }

        ZombieHorseWithInfoRenderState renderState = (ZombieHorseWithInfoRenderState) event.getRenderState();
